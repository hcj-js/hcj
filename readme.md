...
Primitive components are DOM nodes, that is tags and text nodes.  This library provides components ```a```, ```div```, and ```img```, for those elements.  It also provides a function ```textNode``` which takes a string and returns a componet.  E.g. ```textNode('Hello World')``` is a component.
