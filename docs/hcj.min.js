		...
		setTimeout(function () {
		  ...
		  if (firstPush) {
			mh = function (w) {
			  ...
			  var str = $el.text();
			  return Math.ceil(fontSize * str.length * 0.5 / w);
			};
			setTimeout(function () {
			...
			mh = (config.oneLine && $el.css('line-height').indexOf('px') !== -1 && constant(parseFloat($el.css('line-height')))) ||
			  (config.minHeight && constant(config.minHeight)) ||
			  (measureHeight($el));
		  }
		  stream.push(mwS, mw);
