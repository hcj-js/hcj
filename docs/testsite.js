$(function () {
  ...
  var introduction = function () {
    return [
      ...
      p("Like any language or runtime, the browser has good parts and bad parts.  HTML declares the static content on the site for search engines to index and text-only browsers to display.  Javascript is a full, turing-complete programming language, which can make pages snazzy and interactive, however search engines cannot reliably index Javascript-generated content because they would first need to solve the halting problem.  CSS, by comparison, the red-headed stepchild.  Like HTML, it is total, however search engines generally have no need to index it, since affects how the content is displayed, not the content itself."),
      p("CSS does not contain a full programming language, so the only way for it to express the variety of page layouts it does, is for it to provide an enormous zoo of styles that place elements in each of various ways.  These include `position` styles, `float` styles, and `display` styles including `block`, `inline-block`, `table`, `grid`, and `flexbox` - in addition to styles that modify the box-sizing properties of individual elements, such as `margin`, `padding`, and `border`.  Each of these styles has a certain set of capabilities.  With the advent of css3 flexbox and grid, the capabilities are clearly improving, however there are always layouts that cannot be expressed without Javascript.  Generally speaking, as a CSS programmer one learns a set of incantations to create whatever display mode your designer asks for, and then in 5 years a new feature comes out, invalidating your spellbook, at which point you must begin to write a new one."),
      ...
      p("Imagine if you could <i>implement</i> flexbox, grid, or even your own display styles, and use them in your websites right now, without waiting for browser support to come along and without worrying about fallbacks or polyfills.  This is exactly what HCJ is for.  Basically, HCJ can act as a \"polyfill\" for all current and future CSS styles."),
  ...
  var page = c.all([
    ...
    ])(c.text('Footer Footer Footer')),
  ]));

  window.hcj.rootComponent(page);
});
